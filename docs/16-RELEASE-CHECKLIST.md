...
> **Versão Alvo:** EddyTrader 1.0.0-rc1  
...
A tabela abaixo registra formalmente os gates de qualidade, estabilidade e conformidade arquitetural exigidos para declarar o **EddyTrader 1.0.0-rc1**:
...
| **Regressão Formal FSM** | Execução de 100% dos cenários formais automatizados no Strategy Tester. | **APROVADO [X]** | Suíte `tests/test_fsm_w06.mq5` aprovada com **28/28 PASS** (`W06-01..20`, `W07R-01..06` e `W08R-01..02`). |
| **Hardening de Inputs** | Validação defensiva de todos os parâmetros no `OnInit()` com rejeição segura. | **APROVADO [X]** | Verificação estrita de `InpMaxLoss > 0`, `InpBlockDurationHours` (1 a 168h), `InpTimerIntervalMs` (50 a 5000ms), `InpDeviationPoints` (0 a 500). |
...
| **Versão Final 1.0.0** | Declaração da versão sem o sufixo `-rc1`. | **PENDENTE [ ]** | Não autorizada enquanto o gate `LIVE-01` permanecer em aberto. |
...
O produto **EddyTrader 1.0.0-rc1** atende rigorosamente a todos os requisitos de robustez, engenharia de software defensiva, rastreabilidade e operabilidade.

O sistema é formalmente declarado como **RELEASE CANDIDATE 1 (RC1) APROVADO COM RESSALVA EXTERNA (`LIVE-01`)**.
