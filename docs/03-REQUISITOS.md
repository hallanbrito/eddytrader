...
| **RF-005** | O sistema deve detectar quando o resultado financeiro relevante consolidado for menor ou igual ao negativo do limite monetário diário. | Requisito Aprovado (Seção 2) / W02 (D03) | Essencial | O gatilho de proteção dispara no milissegundo em que $\text{Resultado Relevante} \le -\text{Limite Configurado}$. |
...
| **RF-012** | O sistema deve reiniciar o monitoramento criando uma nova janela de proteção a partir de uma baseline de reabertura após o desbloqueio. | Requisito Aprovado (Seção 2) / W02 (D09) | Essencial | A liberação não apaga o histórico contábil do dia e estabelece nova baseline para evitar rebloqueio imediato no mesmo tick. |
...
| **RF-014** | O sistema deve operar conceitualmente como uma instância única por conta de negociação. | W02 (D14) | Alta | O EA não deve disputar o controle da conta com outras instâncias ativas do EddyTrader. |
...
| **RNF-004** | O comportamento do sistema deve ser seguro, previsível e priorizar a reconstrução determinística de estado via histórico nativo após reinicializações. | RNF / W02 (D12) | Alta | Máquina de estados formal com capacidade de inferir o estado operacional a partir do histórico de negócios da conta. |
