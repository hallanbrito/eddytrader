...
| **RF-016** | O HUD principal deve apresentar o resultado da janela/ciclo operacional ativa sem apagar o resultado diário acumulado. | Feedback do Product Owner / W10.3 | Alta | Os modos compacto e minimizado exibem $W_n(t)=D(t)-B_n$. O modo detalhado distingue explicitamente Resultado do Ciclo ($W_n$), Resultado do Dia ($D$) e Baseline do Ciclo ($B_n$). Após `REOPENING`, o novo ciclo inicia visualmente em `0,00`, enquanto $D$ permanece íntegro para auditoria e contabilidade. |
