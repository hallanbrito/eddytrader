...
| **GAP-005** | Persistência vs. Reconstrução de Estado após Restart | **RESOLVIDA QUANTO AO MECANISMO TÉCNICO (W05)** | A reconstrução puramente nativa a partir de deals do histórico foi refutada para janelas subsequentes intradiárias ($n \ge 1$), pois o MT5 não armazena o evento de reabertura ($t_{\text{reopen}}$) nem a baseline $B_n$. O mecanismo técnico de persistência de $\mathbf{D}_{\text{min\_recovery}}$ está formalmente definido via **Global Variables do Terminal** (`GlobalVariableSet` / `GlobalVariablesFlush`) com chave prefixada por login (`EDDY_<LOGIN>_`), e arquivo local binário como contingência. A validação integrada do ciclo de vida completo com o EA real será executada na W06. | [12-SPIKE-TECNICO-MT5.md](file:///C:/Projetos/eddytrader/docs/12-SPIKE-TECNICO-MT5.md) / [ADR 0005](file:///C:/Projetos/eddytrader/docs/adr/0005-garantias-tecnicas-mt5-e-estrategia-de-recuperacao.md) |
...
* **Status:** **RESOLVIDA QUANTO AO MECANISMO TÉCNICO (W05)**
* **Resolução Técnica da W05:** O Spike Técnico comprovou que a memória RAM de um EA é volátil no `OnInit()` e que o histórico nativo do MT5 não registra quando ocorreu um evento de reabertura ($t_{\text{reopen}}$) nem o valor da baseline $B_n$ para janelas subsequentes ($n \ge 1$), tornando a reconstrução puramente nativa inviável para evitar falsos rebloqueios imediatos pós-restart.
* **Decisão Adotada:** O conjunto conceitual mínimo $\mathbf{D}_{\text{min\_recovery}} = \{ \text{current\_state}, \text{protection\_event\_id}, J_n, B_n, t_{\text{trigger}}, t_{\text{unlock}} \}$ tem seu mecanismo técnico definido via **Global Variables do Terminal MT5** (`GlobalVariableSet` / `GlobalVariablesFlush`) com chave prefixada por login (`EDDY_<LOGIN>_<VAR>`), suportando reinicializações do terminal, com persistência binária local (`MQL5/Files`) como contingência. A validação integrada do ciclo de vida em mercado ativo será executada na W06.
* **Documentos Vinculados:** [12-SPIKE-TECNICO-MT5.md](file:///C:/Projetos/eddytrader/docs/12-SPIKE-TECNICO-MT5.md), [ADR 0005](file:///C:/Projetos/eddytrader/docs/adr/0005-garantias-tecnicas-mt5-e-estrategia-de-recuperacao.md), [11-MAQUINA-DE-ESTADOS.md](file:///C:/Projetos/eddytrader/docs/11-MAQUINA-DE-ESTADOS.md).
...
| :--- | :--- | :--- | :--- |
| **DQ-001** | Mecanismo Técnico de Bloqueio Operacional no MT5 | **RESOLVIDA DOCUMENTALMENTE (W05)** | Comprovado que o MT5 não possui interceptação prévia de ordens manuais. A garantia técnica normativa é a **neutralização reativa** via `OnTradeTransaction` emitindo `PositionClose` / `OrderDelete`. A latência real de rede permanece pendente de medição em Demo. |
| **DQ-002** | Tratamento de Contas Netting vs. Hedging | **RESOLVIDA DOCUMENTALMENTE (W05)** | Definida a liquidação universal estritamente por **`POSITION_TICKET`**, eliminando ambiguidade e garantindo operação 100% idêntica e compatível tanto em Netting quanto em Hedging. |
| **DQ-003** | Política de Slippage e Deviation em Fechamento de Emergência | **ADIADA PARA W06** | Parametrização do desvio máximo aceitável em pontos (`deviation`) nas chamadas `CTrade` do primeiro EA mínimo. |
| **DQ-004** | Tratamento de Ativos com Mercado Fechado | **PARCIALMENTE RESOLVIDA (W05)** | Tratamento arquitetural e retcodes mapeados no código do probe (retenção estrita em `LIQUIDATING` com retry via `OnTimer`). Comportamento empírico com servidor remoto pendente de validação em Demo. |
| **DQ-005** | Detecção e Prevenção de Múltiplas Instâncias do EA | **RESOLVIDA DOCUMENTALMENTE / LOCALMENTE (W05)** | Guarda de instância única por conta validada através de flag em GlobalVariable (`EDDY_<LOGIN>_INSTANCE_ACTIVE`) ou identificador de chart. |
