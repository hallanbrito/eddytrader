...
| **GAP-005** | Persistência vs. Reconstrução de Estado após Restart | **PENDENTE (DEFINIDO CONCEITUALMENTE NA W04)** | Dados conceituais mínimos identificados pela FSM ($\mathbf{D}_{\text{min\_recovery}} = \{ \text{current\_state}, \text{protection\_event\_id}, J_n, B_n, t_{\text{trigger}}, t_{\text{unlock}} \}$). Mecanismo técnico de recuperação (histórico nativo puro vs. persistência leve) permanece pendente para validação no Spike Técnico da W05. | [11-MAQUINA-DE-ESTADOS.md](file:///C:/Projetos/eddytrader/docs/11-MAQUINA-DE-ESTADOS.md) / [ADR 0004](file:///C:/Projetos/eddytrader/docs/adr/0004-maquina-de-estados-e-recuperacao.md) |
...
* **Status:** **PENDENTE — Requisitos conceituais definidos na FSM (W04); mecanismo técnico pendente para W05.**
* **Resolução Conceitual da W04:** A especificação da FSM definiu com exatidão o conjunto mínimo de dados necessário para que o estado do sistema seja deterministicamente reconstituído após reinicialização:
  $$\mathbf{D}_{\text{min\_recovery}} = \{ \text{current\_state}, \text{protection\_event\_id}, J_n, B_n, t_{\text{trigger}}, t_{\text{unlock}} \}$$
* **Questão Aberta Remanescente:** A W04 delimitou a necessidade conceitual dessas variáveis, sem antecipar implementações ou formatos físicos. Permanece pendente para o Spike Técnico da W05 determinar **onde e como** essas informações serão armazenadas/reconstituídas (se unívocas via histórico de ordens/deals nativo do MT5 ou se requererão mecanismo auxiliar leve em disco / GlobalVariables).
* **Documentos Vinculados:** [11-MAQUINA-DE-ESTADOS.md](file:///C:/Projetos/eddytrader/docs/11-MAQUINA-DE-ESTADOS.md#10-dados-conceituais-mínimos-para-reconstrução-resolução-formal-do-gap-005-na-w04), [ADR 0004](file:///C:/Projetos/eddytrader/docs/adr/0004-maquina-de-estados-e-recuperacao.md).
...
| **DQ-005** | Detecção e Prevenção de Múltiplas Instâncias do EA | **CONCEITUALIZADA (W04) / VALIDAÇÃO TÉCNICA NA W05** | A FSM formalizou a regra conceitual de instância única (RN-011, FSM-INV-001). A comprovação de mecanismos nativos de guarda (ex.: GlobalVariables ou enumeração de charts) será validada no Spike Técnico da W05. |
