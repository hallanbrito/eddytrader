...
* **Decisão W11-DEC-02:** Se a posição nascer sem SL, o sistema aguarda o primeiro SL válido definido pelo trader, sem calcular um nível inicial. Se a entrada já chegar com SL anexado, esse valor se torna a referência monotônica quando a posição existir. TP permanece fora do lock. OCO entre duas entradas não integra o escopo produtivo da W11.
...
| **DQ-007** | Baseline Inicial do Stop Loss Monotônico | **DECIDIDA PELO PO (W11) / VALIDAÇÃO TÉCNICA PENDENTE** | Sem SL inicial, aguardar a primeira definição válida pelo trader. Com SL anexado à entrada, capturá-lo como referência assim que a posição existir. Não calcular SL automático, não bloquear TP e não incluir OCO entre entradas no escopo produtivo atual. |
